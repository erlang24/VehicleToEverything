...
class UdpArrayDecoderNode(Node):
    def __init__(self):
        ...
        self.other_road_info_pub = self.create_publisher(String, '/other_vehicle_on_road_info', 10)  # 其他车是否在路上
    ...
    def car_message(self,decoded_data):
        ...
        # gps_msg = GPSFix()
        self.gps_msg.latitude = bsm_data['pos']['lat'] / 10000000.0
        self.gps_msg.longitude = bsm_data['pos']['long'] / 10000000.0
        self.gps_msg.speed = bsm_data['speed'] * 0.02 * 3.6
        self.gps_msg.track = bsm_data['heading'] * 0.0125
        print("其他车辆的透传过来的信息：", self.gps_msg)
        self.publisher_car_message.publish(self.gps_msg)
        ...
        if decoded_data[0] == 'mapFrame':
            ...
            for node in map_data.get('nodes', []):
                ...
                for link in node.get('inLinks', []):
                    ...
                    other_is_vehicle_on_lane = False
                    ...
                    for lane in link.get('lanes', []):
                        ...
                        for idx, lane_point in enumerate(lane.get('points', []), start=1):
                            ...
                            if lane_offset_type in ['position-LL1', 'position-LL2']:
                                ...
                                other_car_distance = self.calculate_distance((self.gps_msg.latitude,self.gps_msg.longitude), (actual_lane_lat,actual_lane_lon))
                                
                                if other_car_distance< closest_distance:
                                    closest_distance = other_car_distance
                                    closest_point = (actual_lane_lat, actual_lane_lon)
                                    point_index = idx
                                if self.gps_msg.latitude and (other_car_distance<4):
                                    other_is_vehicle_on_lane = True

                                # print(self.gps_msg.latitude,111111111111111111111111111111)
                        ...
                        if other_is_vehicle_on_lane:
                            self.phase_id = link.get('movements', [{}])[0].get('phaseId', None)
                            print(f"其他车辆在车道上,其他车亮所在车道红绿灯相位ID: {self.phase_id}")
                            self.publish_other_road_info()
                        else:
                            self.phase_id = None
    ...
    def publish_other_road_info(self):
        """发布其他车辆所在道路"""
        if self.phase_id is not None and self.gps_msg.latitude is not None:
            carlane_phase_id = self.phase_id
            message = f"车道相位id:{carlane_phase_id}"
            msg = String()
            msg.data = message
            self.other_road_info_pub.publish(msg)
            self.get_logger().info("bbbbbbbbbbbbbbbbbb其他车辆在指定道路上")
