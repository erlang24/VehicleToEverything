...
if [ "$(uname -m)" = "aarch64" ]; then
  ...
  IMAGE_NAME="pm-pilot-stage1-promote-universe"
  TAG_PREFIX="humble-latest-cuda-arm64-autopilot"
  RUNTIME="--runtime=nvidia"
fi
...
if [ "$(uname -m)" = "x86_64" ]; then
  echo "platform: amd64"
  IMAGE_NAME="pm-pilot-stage1-promote-universe"
  TAG_PREFIX="humble-latest-cuda-amd64-autopilot"
fi
